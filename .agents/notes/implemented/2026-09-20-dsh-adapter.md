...
Cost: a second harness home to explain and keep working, and a one-shot half with no live caller, which ages on its own test coverage alone; pinning the session contract also exposed a shared JSON-RPC framing defect, fixed for every dialect in [2026-09-20-jsonrpc-method-first-routing.md](2026-09-20-jsonrpc-method-first-routing.md); dsh moves fast (22 versions in six weeks), so the adapter is pinned in documentation to a verified release with a smoke checklist rather than a version range; and when `--json`/`--session-id` land for the headless profile, the one-shot parse must move to NDJSON.
