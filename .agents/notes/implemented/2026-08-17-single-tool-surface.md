...
Costs: `setActiveTools` becomes session state (`/reload` keeps the flags on the shared registry), and a host without the tool-list API parks nothing — the four stay active and the announcement is still true. The seven tools measure 5,977 chars; 2,915 of them are paid on every request. See `.agents/notes/implemented/2026-09-24-lazy-tool-activation.md` and `2026-09-24-thin-tool-surface-skill-delegation.md`.
