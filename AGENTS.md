...
- **Permission tiers are enforced mechanically** — by the target CLI harness, or by the session driver answering protocol permission requests (dsh's ACP escalation deny) — never by a prompt-level request. See [docs/adapters.md](docs/adapters.md).
