...
- **Qoder steering and its transport**: Qoder is driven over its documented streaming input channel — `qodercli -p --output-format stream-json --input-format stream-json`, the exact argv the official SDK builds in `buildArgs()` — instead of `--acp`. The ACP page documents only editor integration and exposes no steering metadata there, so a second `session/prompt` under ACP would prove queueing, not step-boundary steering.
  ...
  - Boot sends the SDK's `initialize` control request as soon as the process is up and then waits for the CLI's eager `system`/`init` record before sending the first user message. A failure frame that arrives together with the handshake makes `start()` throw instead of opening a turn on a dead session.
  ...
  Compatibility: `@qoder-ai/qoder-agent-sdk` 1.0.39 pairs with qodercli 1.1.49, and the per-command `command_lifecycle` ack only exists in that newer generation — local qodercli 1.0.18 does not emit it, so nothing waits on it. 1.0.18 does carry the documented `priority: ["now","next","later"]` field on inbound user messages; `shouldQuery` is declared by the SDK and documented, but 1.0.18's inbound schema does not name it, so the "a steer cannot become its own turn" guarantee is only certain on a CLI generation that honours the field.
