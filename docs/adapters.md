...
All eight agents share one dispatch path; the matrix records what each adapter forwards. An omitted `mode` takes the adapter default; an out-of-range request is refused.
...
`enforcesReadOnly` is `false` only for kimi; `degraded` marks a callable-but-known-degraded upstream for tool output (claude is fixture-tested only). Without a sandbox, `write`/`yolo` are permission-rule tiers, not an OS boundary; a readonly hook is a heuristic filter.

## kimi: yolo-only enforcement

`kimi-code` rejects every permission flag alongside `-p`, so a headless run always executes under `default_permission_mode` from `~/.kimi-code/config.toml`; `minMode: "yolo"` refuses the tiers that would carry no enforcement behind the label, and an `effort` request is refused because no reasoning-effort control exists.
...
Every dsh spawn carries `DSH_HOME=~/.dsh-external-agent`, provisioned lazily by `ensureDshHome()`, with `.credentials.yaml` symlinked to the user's own file. That home is load-bearing: `~/.dsh/settings.yaml`'s `permission.defaultPreset` outranks `DSH_PERMISSION_MODE` and `--patch` alike, so the shared home would leave the requested tier unenforced. The symlink keeps rotating credentials in one place; an unsigned-in user gets the `dsh web` instruction instead.
...
Verified against dsh 0.1.5-rc.2. dsh ships fast, but its automation surface (headless contract, ACP, `DSH_*` environment, sandbox vocabulary) stays stable across releases; upgrades re-run the smoke checklist — headless text and exit code, ACP handshake, `set_config_option`, read-only write denial. One-shot answers are plain stdout (`--json`/`--session-id` do not exist yet); the parse moves to NDJSON on arrival.
