 ...
 * keeps a single source of truth, and the link is best-effort. Verified against
 * 0.1.5-rc.2: a completely EMPTY harness home completes `dsh --profile headless`
 * on the default deepseek-official route with no credentials at all, a real
 * user's ~/.dsh/.credentials.yaml holds only the web UI's browser-session record
 * rather than a provider key, and dsh reads project and user `.env` fallbacks
 * too. So the file matters only when a provider's auth actually lives in it, and
 * its absence warns instead of failing.
 *
...
/**
 * The warning an unlinked home carries when the user has no credentials file to
 * link. Not a failure: the default provider route and the `.env` fallbacks can
 * carry auth by themselves, so the run proceeds without the link and the text
 * names both what then has to carry auth and the command that manages the file.
 */
export function dshCredentialsMissingWarning(credentialsSource: string): string {
	return (
		`no ${credentialsSource} to link; proceeding without linked credentials — ` +
		"the default provider route or .env must carry auth (run `dsh web` once to manage credentials)."
	);
}
 ...
 * the user's credentials whenever there is one to link.
 *
 ...
 * target means dsh gets no credentials); a real file or directory at that name
 * is left untouched, because it is not ours to delete — and when it is a real
 * file it is reported as the fork it is.
 */
export function ensureDshHome(options: DshHomeOptions = {}): DshHomeResult {
	...
	if (!existsSync(credentialsSource)) {
		// Nothing to link, and nothing to fail over either. Our own link to that
		// target would only dangle — a credentials file dsh reads and does not
		// find — so it is removed, leaving the home in the state verified to run
		// credential-less (default route, or a `.env` fallback). Anything else at
		// that name is the user's arrangement and stays untouched.
		if (isLinkTo(credentials, credentialsSource)) unlinkIfPresent(credentials);
		return { ok: true, home, credentials, warning: dshCredentialsMissingWarning(credentialsSource) };
	}
...
/** Remove a stale link of ours; best effort, like the mode fix. */
function unlinkIfPresent(credentials: string): void {
	try {
		unlinkSync(credentials);
	} catch {
		/* best effort: a dangling link is not worth failing provisioning over */
	}
}
